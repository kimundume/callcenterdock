...
export class EmailService {
  ...
  static storeToken(email: string, token: string, type: 'email' | 'password'): void {
  ...
  // Verify token
  static verifyToken(email: string, token: string, type: 'email' | 'password'): boolean {
  ...
  static async sendEmailVerification(email: string, companyName: string, token: string): Promise<boolean> {
    try {
  ...
  static async sendPasswordReset(email: string, username: string, token: string): Promise<boolean> {
    try {
  ...
  static async sendCompanyUuidReminder(email: string, companyName: string, uuid: string): Promise<boolean> {
    try {
      const mailOptions = {
        from: `"CallDocker" <${emailConfig.auth.user}>`,
        to: email,
  ...
  static async sendAgentInvitation(email: string, agentUsername: string, companyName: string, uuid: string, tempPassword: string): Promise<boolean> {
    try {
